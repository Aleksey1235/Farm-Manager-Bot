...
class AssignedAmountView(discord.ui.View):
    """Disabled visual state after a manual amount has been assigned."""
    def __init__(self, bot, user_id: int, day: str, amount: int):
        super().__init__(timeout=None)
        button = discord.ui.Button(
            label=f"Назначено {amount:,}$".replace(",", " "),
            emoji="✅",
            style=discord.ButtonStyle.secondary,
            disabled=True,
            custom_id=f"daily_amount_done:{user_id}:{day.replace('-', '')}",
        )
        self.add_item(button)
...
class MainPanelView(discord.ui.View):
    ...
    async def finish(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            now = datetime.now(self.bot.config.timezone)
            result = await self.bot.db.finish_farm(interaction.user.id, now, self.bot.config.timezone)
            await self.bot.db.audit(now, "farm_finish", interaction.user.id, interaction.user.id, str(result))
            if result["status"] == "auto":
                text = f"✅ Фарм завершён.\nСегодня: **{format_duration(result['seconds'])}**\nДневное начисление: **{result['amount']:,}$** автоматически.".replace(",", " ")
            elif result["status"] == "manual":
                text = f"✅ Фарм завершён.\nСегодня: **{format_duration(result['seconds'])}**\nТекущая назначенная сумма: **{result['amount']:,}$**.".replace(",", " ")
            else:
                text = f"✅ Фарм завершён.\nСегодня: **{format_duration(result['seconds'])}**\n⏳ Меньше порога — руководство назначит сумму персонально."
                await self.bot.notify_pending_amount(interaction.user, now.date().isoformat(), result["seconds"])
            await interaction.response.send_message(text, ephemeral=True)
            await self.bot.refresh_panels()
        except Exception as e:
            await interaction.response.send_message(f"❌ {e}", ephemeral=True)
    ...
    async def _finish(self, interaction):
        ...
        try:
            ...
            if result["status"] == "auto":
                ...
                if member:
                    await self.bot.notify_pending_amount(member, now.date().isoformat(), result["seconds"])
            for child in self.children:
                child.disabled = True
            await interaction.response.send_message(text, ephemeral=True)
            try:
