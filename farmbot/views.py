...
class AssignedAmountView(discord.ui.View):
    ...
    async def _keep(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(f"✅ Сумма за {self.day} оставлена: **{self.amount:,}$**.".replace(",", " "), ephemeral=True)
    ...
    async def finish(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            if result["status"] == "auto":
                ...
                text = (
                    f"✅ Фарм завершён.\n"
                    f"Сегодня: **{format_duration(result['seconds'])}**\n"
                    f"Текущая назначенная сумма: **{result['amount']:,}$**.\n"
                    f"🔄 Руководству отправлено обновление для пересмотра суммы."
                ).replace(",", " ")
                ...
                await self.bot.notify_pending_amount(interaction.user, result["work_day"], result["seconds"])
            await interaction.response.send_message(text, ephemeral=True)
            await self.bot.refresh_panels()
        except Exception as e:
            await interaction.response.send_message(f"❌ {e}", ephemeral=True)
