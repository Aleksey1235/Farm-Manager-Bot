...
class FarmBot(commands.Bot):
    ...
    def main_panel_embed(self):
        ...
        e.set_footer(text="FARM Manager Bot v2.7")
        return e
    ...
    async def notify_pending_amount(self, member: discord.Member, day: str, seconds: int):
        ...
        if existing:
            try:
                old_channel = self.get_channel(existing["channel_id"])
                if isinstance(old_channel, discord.TextChannel):
                    message = await old_channel.fetch_message(existing["message_id"])
                    async with self.db.connect() as db:
                        rr = await (await db.execute("SELECT manual_amount FROM daily_reports WHERE user_id=? AND day=?", (member.id, day))).fetchone()
                    manual = int(rr["manual_amount"]) if rr and rr["manual_amount"] is not None else None
                    if manual is not None:
                        embed.title = "🔄 Игрок дофармил — проверьте сумму"
                        embed.description += (f"\n\nРанее назначено: **{manual:,}$**. ".replace(",", " ") + "Общее время изменилось. Оставьте прежнюю сумму или измените её.")
