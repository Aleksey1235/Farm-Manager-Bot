...
class FarmBot(commands.Bot):
    ...
    async def my_day_embed(self, user_id: int, display_name: str):
        now = datetime.now(self.config.timezone)
        today = now.date()
        seconds = await self.db.farm_seconds_for_day(user_id, today, self.config.timezone, now)
        threshold = await self.db.setting_int("farm_threshold_minutes")
        ...
        today_row = next((x for x in summary["days"] if x["day"] == today), None)
        contents_today = 0
        ...
        e = discord.Embed(title=f"📋 {display_name} • Сегодня")
        state_text = {
            "idle": "⚪ Не фармит",
