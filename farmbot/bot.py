...
from .views import MainPanelView, AdminPanelView, ContentReviewView, PaymentReviewView, PendingAmountView, AssignedAmountView, SessionReminderView
...
class FarmBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        for row in await self.db.latest_daily_report_reviews():
            if row["manual_amount"] is None:
                self.add_view(PendingAmountView(self, row["user_id"], row["day"]))
            else:
                self.add_view(
                    AssignedAmountView(
                        self, row["user_id"], row["day"], int(row["manual_amount"])
                    )
                )
    ...
    def main_panel_embed(self):
        ...
        e.set_footer(text="FARM Manager Bot v2.8")
        return e
    ...
    async def resolve_text_channel(self, channel_id: int, purpose: str = "канал"):
        channel = self.get_channel(channel_id)
        if channel is None:
            try:
                ...
                raise RuntimeError(
                    f"Не удалось открыть {purpose} (ID {channel_id}). "
                    f"Проверьте ID и права бота."
                ) from exc
        if not isinstance(channel, discord.TextChannel):
            raise RuntimeError(
                f"{purpose.capitalize()} с ID {channel_id} не является текстовым каналом."
            )
        return channel

    async def notify_pending_amount(self, member: discord.Member, day: str, seconds: int):
        """Always send a fresh leadership report for a sub-threshold finish."""
        channel = await self.resolve_text_channel(
            self.config.report_channel_id, "канал FARM-отчётов"
        )
        threshold = await self.db.setting_int("farm_threshold_minutes")

        async with self.db.connect() as db:
            row = await (await db.execute("""
                SELECT manual_amount,status
                FROM daily_reports
                WHERE user_id=? AND day=?
            """, (member.id, day))).fetchone()
        ...
        if manual is None:
            embed = discord.Embed(
                ...
                description=(
                    ...
                    f"Рабочий день: **{day}**\n"
                    f"Суммарный чистый фарм: **{format_duration(seconds)}**\n"
            ...
            embed = discord.Embed(
                title="🔄 Повторное завершение фарма — проверьте сумму",
                description=(
                    f"Игрок: {member.mention}\n"
                    f"Рабочий день: **{day}**\n"
                    f"Новое суммарное время: **{format_duration(seconds)}**\n"
                    f"Порог: **{threshold//60}ч {threshold%60}м**\n\n"
                    f"Сейчас назначено: **{manual:,}$**.\n"
                    f"Игрок сделал ещё одну смену, но суммарно всё ещё ниже порога.\n"
                    f"Оставьте прежнюю сумму или измените её."
                ).replace(",", " ")
            )
            view = AssignedAmountView(self, member.id, day, manual)

        embed.set_footer(
            text="Каждое завершение ниже порога создаёт новый свежий отчёт."
        )

        self.add_view(view)
        message = await channel.send(embed=embed, view=view)
        await self.db.save_daily_report_notification(
            member.id, day, channel.id, message.id
        )

        log.info(
            "Daily FARM report sent user=%s day=%s seconds=%s channel=%s message=%s manual=%s",
            member.id, day, seconds, channel.id, message.id, manual
        )
        return message
