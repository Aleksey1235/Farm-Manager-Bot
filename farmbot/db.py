...
class Database:
    ...
    async def start_farm(self, user_id: int, now: datetime):
        async with self._lock:
            async with self.connect() as db:
                ...
                await db.execute("""
                    UPDATE states SET state='farming', farming_started_at=?, content_started_at=NULL,
                    content_return_state=NULL, updated_at=? WHERE user_id=?
                """, (now.isoformat(), now.isoformat(), user_id))
                await db.execute("""
                    INSERT INTO farm_segments(user_id,started_at) VALUES(?,?)
                """, (user_id, now.isoformat()))
                await db.commit()
        ...
        async with self._lock:
            async with self.connect() as db:
                ...
                if next_state == "farming":
                    ...
                    farm_started = None
                await db.execute("""
                    UPDATE states SET state=?, farming_started_at=?, content_started_at=NULL,
                    content_return_state=NULL, updated_at=? WHERE user_id=?
                """, (next_state, farm_started, now.isoformat(), user_id))
                await db.commit()
    ...
    async def finish_farm(self, user_id: int, now: datetime, tz) -> dict[str, Any]:
        async with self._lock:
            async with self.connect() as db:
                ...
                await db.execute("""
                    UPDATE states SET state='idle', farming_started_at=NULL, updated_at=? WHERE user_id=?
                ...
                await db.commit()
        total = await self.farm_seconds_for_day(user_id, now.astimezone(tz).date(), tz, now)
        ...
        if total >= threshold * 60:
            async with self.connect() as db:
                ...
                """, (user_id, day))
                await db.commit()
            return {"seconds": total, "status": "auto", "amount": auto_amount}
        async with self.connect() as db:
            rr = await (await db.execute("SELECT manual_amount FROM daily_reports WHERE user_id=? AND day=?",
                                         (user_id, day))).fetchone()
        return {"seconds": total, "status": "manual" if rr and rr["manual_amount"] is not None else "pending",
    ...
    async def farm_seconds_for_day(self, user_id: int, day: date, tz, now: datetime) -> int:
        ...
        async with self.connect() as db:
            rows = await (await db.execute("""
                ...
                WHERE user_id=? AND started_at < ? AND COALESCE(ended_at, ?) > ?
            """, (user_id, day_end.isoformat(), now.isoformat(), day_start.isoformat()))).fetchall()
        total = 0
        for r in rows:
            s = max(datetime.fromisoformat(r["started_at"]), day_start)
            e0 = datetime.fromisoformat(r["ended_at"]) if r["ended_at"] else now
            e = min(e0, day_end)
    ...
    async def active_states(self):
        async with self.connect() as db:
            rows = await (await db.execute("""
                SELECT m.user_id,m.display_name,s.state,s.farming_started_at,s.content_started_at
