...
class Database:
    ...
    async def latest_daily_report_reviews(self):
        async with self.connect() as db:
            rows = await (await db.execute("""
                SELECT n.user_id,n.day,n.channel_id,n.message_id,
                       d.status,d.manual_amount
                FROM daily_report_notifications n
                JOIN daily_reports d
                  ON d.user_id=n.user_id AND d.day=n.day
                WHERE d.status IN ('pending','manual')
                ORDER BY d.day DESC
            """)).fetchall()
            return [dict(r) for r in rows]
